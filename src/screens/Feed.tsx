      ...
      <div className="feed">
        {moods.map((m, idx) => {
          ...
          return (
            <article
              key={m.id}
              className={'mood' + (playing === m.id ? ' playing' : '')}
              style={{ ['--a' as string]: m.accent }}
            >
              <div className="mood__frame" style={{ ['--ar' as string]: ar }} onPointerDown={() => play(m)}>
                <KeebScene accent={m.accent} seed={m.id.charCodeAt(1)} playing={playing === m.id} />
                <div className="mood__scrim" />
